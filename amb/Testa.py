...
def callback1():
    ...
    try:
        x = task.index("a1")
        topic = "/hshl/ambulances/arrived"
        payload = ("a1"+" "+str(task[1+x]))
...
def callback11():
    ...
    try:
        x = task.index("a1")
        topic = "/hshl/ambulances/returnvehicle"
        payload = ("a1"+" "+str(task[x+2]))
        client.publish(topic, str(payload))
        task.remove(task[x])
        task.remove(task[x])
        task.remove(task[x])
...
def on_connect(client, userdata, flags, rc):
    client.subscribe('/hshl/ambulances/sendve')
