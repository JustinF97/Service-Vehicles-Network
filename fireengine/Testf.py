...
def callback1():
    ...
    try:
        x = task.index("f1")
        topic = "/hshl/firefighters/arrived"
        payload = ("f1"+" "+str(task[1+x]))
...
def callback11():
    ...
    try:
        x = task.index("f1")
        topic = "/hshl/firefighters/returnvehicle"
        payload = ("f1"+" "+str(task[x+2]))
        client.publish(topic, str(payload))
        task.remove(task[x])
        task.remove(task[x])
        task.remove(task[x])
...
def on_connect(client, userdata, flags, rc):
    client.subscribe('/hshl/firefighters/sendve')
